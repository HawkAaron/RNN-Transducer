...
parser.add_argument('--out', type=str, default='exp/rnnt_1e-3',
...
model = Transducer(dropout=args.dropout, ret=True)
...
def evaluate(ctx=context):
    ...
    for i, (k, v) in enumerate(kaldi_io.read_mat_ark(cv_feat)):
        ...
        am, pm, loss = model(xs, ys, xlen, ylen)
        losses.append(float(loss.sum().asscalar()))
        am = am.asnumpy(); pm = pm.asnumpy(); ys = ys.asnumpy()
        tacc.update(am, ys); facc.update(pm, np.concatenate((ys, np.zeros((ys.shape[0], 1))), axis=1))
    return sum(losses) / len(losses) / args.batch_size, tacc.get(), facc.get()
    ...
    for epoch in range(1, args.epochs):
        ...
        for i, (k, v) in enumerate(kaldi_io.read_mat_ark(train_feat)):
            ...
            if i % args.log_interval == 0 and i > 0:
                ...
                logging.info('[Epoch %d Batch %d] loss %.2f, am PER %.2f, pm PER %.2f'%(epoch, i, l0, tacc.get(), facc.get()))
                totl0 = 0

        losses = sum(losses) / len(losses) / args.batch_size
        val_l, am, pm = evaluate()

        logging.info('[Epoch %d] time cost %.2fs, train loss %.2f, am PER %.2f, pm PER %.2f; cv loss %.2f, am PER %.2f, pm PER %.2f; lr %.2e'%(
            epoch, time.time()-start_time, losses, tacc.getAll(), facc.getAll(), val_l, am, pm, trainer.learning_rate))
